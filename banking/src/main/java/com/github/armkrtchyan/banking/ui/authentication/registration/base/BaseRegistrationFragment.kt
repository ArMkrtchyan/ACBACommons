...
import com.github.armkrtchyan.banking.ui.authentication.registration.RegistrationViewPagerFragment
...
abstract class BaseRegistrationFragment<VB : ViewBinding, VIEWMODEL : BaseViewModel> : BaseViewBindingViewModelFragment<VB, VIEWMODEL>() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (requireParentFragment() is NavHostFragment) {
            if ((requireParentFragment() as NavHostFragment).childFragmentManager.fragments[0] is RegistrationViewPagerFragment)
                mNextButton.setOnClickListener((requireParentFragment() as NavHostFragment).childFragmentManager.fragments[0] as RegistrationViewPagerFragment)
        }
    }
}