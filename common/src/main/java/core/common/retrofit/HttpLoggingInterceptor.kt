        ...
        if (logHeaders) {
            ...
            if (!logBody || requestBody == null) {
                ...
                if (!buffer.isProbablyUtf8()) {
                    ...
                    try {
                        logger.log(mGson.toJson(JsonParser.parseString(buffer.readString(charset))))
                    } catch (e: Exception) {
        ...
        if (logHeaders) {
            ...
            if (!logBody || !response.promisesBody()) {
                ...
                if (contentLength != 0L) {
                    logger.log("")
                    try {
                        logger.log(mGson.toJson(JsonParser.parseString(buffer.clone().readString(charset))))
                    } catch (e: Exception) {
